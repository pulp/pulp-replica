...
from pulp_replica.app.serializers import ReplicaContentSerializer
from pulp_replica.app.models import ReplicaContent

from pulpcore.plugin.models import Artifact


# Fill data with sufficient information to create ReplicaContent
# Provide sufficient parameters to create the ReplicaContent object
# Depending on the base class of the serializer, provide either "_artifact" or "_artifacts"
@unittest.skip("FIXME: plugin writer action required")
class TestReplicaContentSerializer(TestCase):
    """Test ReplicaContentSerializer."""

    def setUp(self):
        """Set up the ReplicaContentSerializer tests."""
    ...
    def test_valid_data(self):
        """Test that the ReplicaContentSerializer accepts valid data."""
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ReplicaContentSerializer(data=data)
        self.assertTrue(serializer.is_valid())

    def test_duplicate_data(self):
        """Test that the ReplicaContentSerializer does not accept data."""
        ReplicaContent.objects.create(artifact=self.artifact)
        data = {"_artifact": "/pulp/api/v3/artifacts/{}/".format(self.artifact.pk)}
        serializer = ReplicaContentSerializer(data=data)
