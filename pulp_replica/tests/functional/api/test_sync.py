"""Tests that sync replica plugin repositories."""
import unittest
...
from pulp_replica.tests.functional.constants import (
    REPLICA_FIXTURE_SUMMARY,
    REPLICA_INVALID_FIXTURE_URL,
)
...
class BasicSyncTestCase(unittest.TestCase):
    """Sync a repository with the replica plugin."""

    @classmethod
    def setUpClass(cls):
        """Create class-wide variables."""
        cls.cfg = config.get_config()
        cls.client = gen_replica_client()

    def test_sync(self):
        """Sync repositories with the replica plugin.
        ...
        8. Assert that the same number of are present and that no units were
           added.
        """
        repo_api = RepositoriesReplicaApi(self.client)
        remote_api = RemotesReplicaApi(self.client)
        ...
        self.assertDictEqual(get_content_summary(repo.to_dict()), REPLICA_FIXTURE_SUMMARY)
        self.assertDictEqual(get_added_content_summary(repo.to_dict()), REPLICA_FIXTURE_SUMMARY)
...
class SyncInvalidTestCase(unittest.TestCase):
    ...
    @unittest.skip("FIXME: Plugin writer action required.")
    def test_invalid_replica_content(self):
        ...
        """
        with self.assertRaises(PulpTaskError) as cm:
            task = self.do_test(REPLICA_INVALID_FIXTURE_URL)
        task = cm.exception.task.to_dict()
    ...
    def do_test(self, url):
        ...
        body = gen_replica_remote(url=url)
        remote = remote_api.create(body)
        self.addCleanup(remote_api.delete, remote.pulp_href)
